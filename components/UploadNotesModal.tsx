...
const UploadNotesModal: React.FC = () => {
    ...
    const ALLOWED_TYPES = ['text/plain', 'text/markdown', 'application/json', 'text/csv'];
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            if (!isValidType) {
                setError("Unsupported file format. Please upload text-based files only.");
                setFile(null);
                return;
            }
                ...
                <div className="p-8">
                            ...
                            <div className="mb-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm">
                                    <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                                        <div className="w-1.5 h-1.5 rounded-full bg-green-500"></div> .TXT (Text File)
                                    </div>
                                    <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                                        <div className="w-1.5 h-1.5 rounded-full bg-green-500"></div> .MD (Markdown)
                                    </div>
                                    <div className="flex items-center gap-2 text-slate-400">
                                        <div className="w-1.5 h-1.5 rounded-full bg-slate-300"></div> .PDF (Coming Soon)
                                    </div>
                                    <div className="flex items-center gap-2 text-slate-400">
                                        <div className="w-1.5 h-1.5 rounded-full bg-slate-300"></div> .DOCX (Coming Soon)
                                    </div>
                                </div>
                                <p className="text-[10px] text-slate-400 mt-3 leading-relaxed">
                                    *Currently, we only support raw text formats for direct AI parsing. For PDFs, please copy the text into a .txt file first.
                                </p>
                            </div>
                    ...
                    {(status !== 'idle' && status !== 'success') && (
                        <div className="py-8">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 text-sm">
                                {status === 'uploading' && 'Uploading your notes securely...'}
                                {status === 'parsing' && 'AI is analyzing structure & generating hierarchy...'}
                                {status === 'embedding' && 'Creating vector embeddings for the Mentor...'}
                            </p>
                        </div>
                    )}
                    ...
                    {status === 'success' && (
                        <div className="py-8 animate-in zoom-in duration-300">
                            ...
                            <h3 className="font-bold text-lg text-slate-900 dark:text-white">Analysis Complete!</h3>
                            <p className="text-slate-500 dark:text-slate-400 text-sm">Your roadmap is ready to explore.</p>
                        </div>
                    )}
                </div>
            </div>
