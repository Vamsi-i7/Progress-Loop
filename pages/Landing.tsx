...
const Landing: React.FC = () => {
  ...
  return (
    <div className="min-h-screen font-sans bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white overflow-x-hidden selection:bg-indigo-500 selection:text-white">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            {isAuthenticated ? (
                <div className={`flex items-center gap-3 pl-4 border-l ${scrolled ? 'border-slate-200 dark:border-slate-800' : 'border-white/20'}`}>
                    ...
                    {/* Integrated Functional Notification Dropdown */}
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-white dark:bg-slate-950 p-6 flex flex-col animate-in slide-in-from-right duration-200">
            ...
            <div className="flex flex-col gap-6 text-lg font-medium">
                {!isAuthenticated && (
                    <>
                    <button className="text-left" onClick={() => scrollToSection('features')}>Features</button>
                    <button className="text-left" onClick={() => scrollToSection('stats')}>Stats</button>
                    <button className="text-left" onClick={() => scrollToSection('pricing')}>Pricing</button>
                ...
                {isAuthenticated ? (
                    <>
                        <div className="flex items-center gap-3 mb-4">
                             <div className={`w-10 h-10 rounded-full ${getColorClass(themeColor, 'bg')} text-white flex items-center justify-center text-sm font-bold`}>
                                {user.name.substring(0, 2).toUpperCase()}
                             </div>
                             <div>
                        ...
                        <button onClick={() => navigate('/dashboard')} className="flex items-center gap-3 text-slate-600 dark:text-slate-300">
                            <LayoutDashboard size={20}/> Dashboard
                        </button>
                        <button onClick={() => navigate('/settings')} className="flex items-center gap-3 text-slate-600 dark:text-slate-300">
                            <Settings size={20}/> Settings
                        </button>
                        {/* Mobile Menu still allows logout for accessibility, but main nav doesn't */}
                        <button onClick={handleLogout} className="flex items-center gap-3 text-red-500">
                            <LogOut size={20}/> Log Out
                        </button>
                    </>
                ) : (
                    <>
                         <button onClick={handleSignIn} className="w-full py-3 rounded-xl font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800">
                            Sign In
                        </button>
                        <button onClick={handleGetStarted} className={`w-full py-4 rounded-xl font-bold text-white ${getColorClass(themeColor, 'bg')}`}>
                            Sign Up
                        </button>
                    </>
                )}
      ...
      <section id="stats" className="py-20 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 scroll-mt-20">
          <div className="max-w-7xl mx-auto px-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 text-center">
                  {[
                      { label: "Active Students", value: "+10k", color: "text-indigo-500", icon: Users },
                      { label: "Habits Tracked", value: "+500k", color: "text-purple-500", icon: Activity },
                      { label: "Goals Reached", value: "+50k", color: "text-pink-500", icon: Target },
                      { label: "Countries", value: "100+", color: "text-amber-500", icon: Globe },
                  ].map((stat, i) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 shadow-sm hover:shadow-xl transition-all hover:-translate-y-2 border border-slate-100 dark:border-slate-800 group">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 leading-relaxed">
                          Create your very own study universe. Track goals, visualize progress, and keep your daily tasks front and center.
                      </p>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 shadow-sm hover:shadow-xl transition-all hover:-translate-y-2 border border-slate-100 dark:border-slate-800 group">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 leading-relaxed">
                          Earn XP, maintain streaks, and level up as you complete tasks. Turn studying into a game you actually want to play.
                      </p>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 shadow-sm hover:shadow-xl transition-all hover:-translate-y-2 border border-slate-100 dark:border-slate-800 group">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 leading-relaxed">
                          Never miss a deadline. Structure your study sessions, set priorities, and ace your upcoming exams with ease.
                      </p>
                  </div>
              </div>
          </div>
