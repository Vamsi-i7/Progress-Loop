...
const Subscription: React.FC = () => {
  const { themeColor, user, upgradePlan } = useStore();
  ...
  const handleSubscribeClick = (plan: 'pro' | 'premium') => {
  ...
  const PlanCard = ({ title, price, features, recommended, icon: Icon, planKey }: any) => {
    ...
    return (
      <div className={`relative p-8 rounded-[2rem] border transition-all duration-300 flex flex-col ${recommended 
        ...
        >
          {isCurrentPlan ? 'Current Plan' : title === 'Free' ? 'Forever Free' : 'Subscribe Now'}
        </button>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
      ...
      <main className="p-6 max-w-7xl mx-auto">
        ...
        {/* Enterprise Section */}
        <div className="max-w-4xl mx-auto bg-slate-900 dark:bg-white rounded-[2.5rem] p-8 md:p-12 text-center md:text-left flex flex-col md:flex-row items-center justify-between gap-8 shadow-xl">
           <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800 dark:bg-slate-100 text-slate-300 dark:text-slate-600 text-xs font-bold uppercase tracking-wider mb-4">
                 <Building2 size={14} /> Enterprise
              </div>
              <h3 className="text-2xl md:text-3xl font-bold text-white dark:text-slate-900 mb-2">Need a custom solution?</h3>
              <p className="text-slate-400 dark:text-slate-600 max-w-md">For schools, universities, or large study groups, we offer custom pricing and dedicated support.</p>
           </div>
           <button onClick={() => window.location.href = 'mailto:sales@progressloop.app'} className="px-8 py-4 rounded-xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white font-bold text-lg hover:scale-105 transition-transform flex items-center gap-2 whitespace-nowrap">
              Contact Sales <ArrowRight size={20} />
           </button>
        </div>

        {/* Restore Purchase / Manage Link */}
        ...
        {showPayment && selectedPlan && (
            <PaymentModal 
                plan={selectedPlan}
                amount={selectedPlan === 'pro' 
                    ? (billingCycle === 'monthly' ? PRICING_CONFIG.pro.monthly : PRICING_CONFIG.pro.yearly)
                    : (billingCycle === 'monthly' ? PRICING_CONFIG.premium.monthly : PRICING_CONFIG.premium.yearly)
                }
